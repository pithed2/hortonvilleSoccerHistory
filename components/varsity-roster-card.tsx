...
const fieldStats = [["gp", "Games"], ["goals", "Goals"], ["assists", "Assists"], ["points", "Points"]] as const
const keeperStats = [["gp", "Games"], ["minutes", "Minutes"], ["ga", "Goals against"], ["saves", "Saves"]] as const
...
export function VarsityRosterCard({ player, history }: { player: RosterPlayer; history: VarsitySeasonHistory[] }) {
  ...
  const goalkeepingTotals = totals("goalkeeper", ["gp", "minutes", "ga", "saves"])
  return <details className="group min-w-0 self-start rounded-xl border bg-card open:border-primary/40">
    ...
    <div className="space-y-5 border-t p-4">
      ...
      <section aria-label={`${player.player_name} varsity career totals`} className="rounded-xl border border-primary/20 bg-primary/5 p-3">
        ...
        {history.some(season => season.stats) ? <StatGrid row={scoringTotals} fields={fieldStats} /> : null}
        {history.some(season => season.goalkeeper) ? <><h4 className="mt-3 text-sm font-bold">Goalkeeping totals</h4><StatGrid row={goalkeepingTotals} fields={keeperStats} /></> : null}
