  ...
  return <main className="min-h-screen bg-background">
    ...
    <div className="site-container space-y-8 py-10 sm:py-12">
      ...
      <section className="grid gap-6 xl:grid-cols-[.65fr_1.35fr]">
        ...
        <article id="players" className="surface-card min-w-0 scroll-mt-36 p-5 sm:p-7"><p className="section-eyebrow">Player stats</p><h2 className="text-2xl font-black">Full roster</h2><p className="mt-1 text-xs text-muted-foreground">{jvStats.gamesNote ?? "GP reflects team participation · Goals: 2 points · Assists: 1 point"}</p><p className="mt-1 text-xs text-muted-foreground">Rostered players with no goals or assists still contribute through defending and other play not reflected in these statistics.</p><div className="mt-4 max-h-[36rem] overflow-auto"><Table><TableHeader><TableRow><TableHead>Player</TableHead><TableHead className="text-center">GP</TableHead><TableHead className="text-center">G</TableHead><TableHead className="text-center">A</TableHead><TableHead className="text-right">Pts</TableHead></TableRow></TableHeader><TableBody>{[...jvStats.players].sort((a, b) => compareJerseyNumbers(a.number, b.number) || a.name.localeCompare(b.name)).map(player => <TableRow key={`${player.squad ?? ""}-${player.number}-${player.name}`}><TableCell><div className="flex items-center gap-3"><span className="grid size-8 place-items-center rounded-lg bg-primary/10 text-xs font-black text-primary">{player.number}</span><span><span className="font-semibold">{player.name}</span>{player.squad ? <span className="block text-xs text-muted-foreground">{player.squad}</span> : null}</span></div></TableCell><TableCell className="text-center">{player.gp ?? "—"}</TableCell><TableCell className="text-center">{player.goals ?? "—"}</TableCell><TableCell className="text-center">{player.assists ?? "—"}</TableCell><TableCell className="text-right font-black text-primary">{player.points ?? "—"}</TableCell></TableRow>)}</TableBody></Table></div></article>
      </section>
