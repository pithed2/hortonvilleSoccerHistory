...
export function OnTap({ data, today }: { data: WeeklyData; today: string }) {
  ...
  return (
    <section className="mb-8 rounded-2xl border bg-white p-5 sm:p-6" aria-labelledby="on-tap-title">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-primary">The week’s matchups</p>
          <h2 id="on-tap-title" className="mt-1 text-2xl font-bold">On tap this week</h2>
      ...
      <p className="mt-4 text-sm leading-6 text-muted-foreground">Monday–Sunday · Winning record means more wins than losses. Highlights stay listed after scores are entered. Records reflect the latest saved data; opponents without a known record aren’t classified as key matchups.</p>
      <div className="mt-5 grid items-start gap-5 lg:grid-cols-2">
        {sections.map(section => (
          <section key={section.title} className="overflow-hidden rounded-xl border">
            ...
            {section.games.length ? <ul className="divide-y">{section.games.map(game => (
              <li key={[game.Date, game.Team, game.Opponent].join("|")} className={`flex flex-wrap items-center justify-between gap-3 p-4 ${game.Team === "Hortonville" || game.Opponent === "Hortonville" ? "bg-red-50/60" : ""}`}>
                <div className="min-w-0">
                  ...
                  {game.opponentRecord && <p className="mt-1 text-xs text-muted-foreground">{game.Opponent}: {game.opponentRecord.W}–{game.opponentRecord.L}–{game.opponentRecord.T}</p>}
                </div>
