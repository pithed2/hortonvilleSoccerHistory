...
type Seed = { Team: string; Group: string; GroupPoints: number; OverallPoints: number; GD: number; Rank: number | null; Rating: number | null; Seed: number; Composite: number }
...
function SeedingTable({ rows, team }: { rows: Seed[]; team: string }) { return <Card><CardHeader><CardTitle>Live {rows?.[0]?.Group} seeding</CardTitle></CardHeader><CardContent><Table><TableHeader><TableRow><TableHead>Seed</TableHead><TableHead>Team</TableHead><TableHead>State rank</TableHead><TableHead>Rating</TableHead><TableHead>Group pts</TableHead><TableHead>Overall pts</TableHead><TableHead>GD</TableHead></TableRow></TableHeader><TableBody>{rows?.map(row => <TableRow key={row.Team} className={row.Team === team ? "bg-red-50 font-semibold" : ""}><TableCell><Badge variant={row.Team === team ? "default" : "secondary"}>#{row.Seed}</Badge></TableCell><TableCell>{row.Team}</TableCell><TableCell>{row.Rank ? `#${row.Rank}` : "–"}</TableCell><TableCell>{row.Rating?.toFixed(1) ?? "–"}</TableCell><TableCell>{row.GroupPoints}</TableCell><TableCell>{row.OverallPoints}</TableCell><TableCell>{row.GD > 0 ? `+${row.GD}` : row.GD}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card> }
...
function HeadToHead({ opponents, games, team }: { opponents: Record<string, string | number | null>; games: Game[]; team: string }) {
  ...
  const tones = {
    ...
    L: "border-red-200 bg-red-50 text-red-900",
