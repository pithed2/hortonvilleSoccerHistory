...
export default async function SeasonYearPage({ params }: Props) {
  ...
  return <main className="min-h-screen bg-background"><Navigation />
    ...
    <div className="mx-auto max-w-7xl space-y-14 px-4 py-12 sm:px-6 lg:px-8">
      ...
      {playerStats.length ? <section id="players" className="scroll-mt-36"><div className="mb-6 flex items-center gap-3"><Goal className="h-6 w-6 text-primary" /><h2 className="text-3xl font-black">Player Stats</h2></div><div className="overflow-x-auto rounded-2xl border bg-card shadow-sm"><table className="w-full min-w-[440px] text-sm"><caption className="sr-only">{year} player season statistics</caption><thead className="bg-muted/60"><tr><th scope="col" className={cell}>Player</th><th scope="col" className={cell}>GP</th><th scope="col" className={cell}>G</th><th scope="col" className={cell}>A</th><th scope="col" className={cell}>Pts</th></tr></thead><tbody>{playerStats.map((player) => <tr key={player.player_name} className="even:bg-muted/20"><td className={`${cell} font-bold`}>{player.player_name}</td><td className={cell}>{player.gp}</td><td className={cell}>{player.goals}</td><td className={cell}>{player.assists}</td><td className={`${cell} font-black text-primary`}>{player.points}</td></tr>)}</tbody><tfoot className="bg-primary/5 font-bold"><tr><th scope="row" className={cell}>Recorded totals</th><td className={cell}>—</td>{(["goals", "assists", "points"] as const).map(key => <td key={key} className={cell}>{playerStats.reduce((sum, player) => sum + player[key], 0)}</td>)}</tr></tfoot></table></div></section> : null}
