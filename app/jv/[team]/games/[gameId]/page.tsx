    ...
    <div className="mx-auto max-w-6xl space-y-7 px-4 py-10 sm:px-6 lg:px-8">
      <section className="surface-card overflow-hidden">
        <div className="border-b bg-gradient-to-br from-primary/10 via-background to-background px-5 py-7 sm:px-9 sm:py-9">
          <div className="flex flex-wrap gap-5 text-xs font-semibold uppercase tracking-[.13em] text-muted-foreground"><span className="flex items-center gap-2"><CalendarDays className="size-4 text-primary" />{formatDate(game.date)}{game.kickoff ? ` · ${game.kickoff}` : ""}</span><span className="flex items-center gap-2"><MapPin className="size-4 text-primary" />{game.location}</span>{game.conference ? <span className="rounded-full bg-primary/10 px-3 py-1 text-primary">Conference</span> : null}</div>
...
function formatDate(value: string) { if (!value) return "Date not recorded"; return new Intl.DateTimeFormat("en-US", { month: "long", day: "numeric", year: "numeric", timeZone: "UTC" }).format(new Date(`${value}T00:00:00Z`)) }
