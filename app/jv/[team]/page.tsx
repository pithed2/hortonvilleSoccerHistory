...
export default async function JvTeamPage({ params }: Props) {
  ...
  return <main id="main-content" className="min-h-screen bg-background">
    ...
    <div className="site-container space-y-8 py-10 sm:py-12">
      ...
      {teamSlug === "white" ? <JvCalendar teams={["JV White"]} /> : null}
      <section id="schedule" className="scroll-mt-36 grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
        <article className="surface-card p-5 sm:p-7"><p className="section-eyebrow">Match center</p><h2 className="text-2xl font-black">Recent results</h2><div className="mt-5 space-y-2">{jvStats.recent.map(game => <Link href={`/jv/${teamSlug}/games/${game.id}`} key={game.id} className="group grid grid-cols-[56px_minmax(0,1fr)_auto_auto] items-center gap-3 rounded-xl border bg-card p-4 transition hover:border-primary/40 hover:bg-primary/5"><p className="text-xs font-bold uppercase text-muted-foreground">{game.date}</p><div className="min-w-0"><p className="truncate font-bold">{game.opponent}</p>{isJvConferenceOpponent(game.opponent) ? <span className="text-xs font-semibold text-primary">Conference</span> : null}<p className="truncate text-xs text-muted-foreground">{game.location}{game.kickoff ? ` · ${game.kickoff}` : ""} · Box score</p></div><p className="text-xl font-black">{game.score}</p><ChevronRight className="size-4 text-muted-foreground group-hover:text-primary" /></Link>)}</div></article>
        <article className="surface-card p-5 sm:p-7"><p className="section-eyebrow">Coming up</p><h2 className="text-2xl font-black">Next matches</h2><div className="mt-5 divide-y">{!jvStats.upcoming.length ? <p className="text-sm text-muted-foreground">Upcoming schedule has not been added.</p> : null}{jvStats.upcoming.map((game,index) => <div key={`${game.date}-${game.opponent}`} className="flex items-center gap-4 py-4 first:pt-0 last:pb-0"><div className={`grid size-11 shrink-0 place-items-center rounded-xl text-xs font-black ${index === 0 ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}>{game.date}</div><div className="min-w-0"><p className="truncate font-bold">{game.opponent}</p>{isJvConferenceOpponent(game.opponent) ? <span className="text-xs font-semibold text-primary">Conference</span> : null}<p className="truncate text-xs text-muted-foreground">{game.location}</p></div></div>)}</div></article>
      </section>
