...
export default function JvBlackGrayPage() {
  ...
  const stats: JvDashboardStats = {
    ...
    goalkeepers: [...goalkeeperNames].map(name => {
      const lines = blackGrayData.gameStats.flatMap<{ name: string; minutes: number; saves: number | null }>(game => game.goalkeepers).filter(line => line.name === name)
      return { name, number: players.find(player => player.name === name)!.number,
        ...
        minutes: lines.length ? lines.reduce((sum, line) => sum + line.minutes, 0) : null, goalsAgainst: null }
    }),
  }
